        ...
        >
          ...
          <div className={`w-[calc(50%-2rem)] ${index % 2 === 0 ? 'mr-auto text-right' : 'ml-auto text-left'}`}>
            <div className="rounded-lg bg-blueprint-bg p-6 shadow-lg transition-all duration-300 group-hover:border-primary/50 group-hover:shadow-primary/10 group-hover:-translate-y-1">
              <p className="font-mono text-sm font-semibold text-muted-foreground">{item.date_from} - {item.date_to}</p>
              <h3 className="text-xl font-bold text-foreground">{item.subtitle}</h3>
              {item.link_url ? (
                <Link href={item.link_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-md font-medium text-primary transition-opacity hover:opacity-80">
              ...
              {item.tags && item.tags.length > 0 && (
                <div className={`mt-4 flex flex-wrap gap-2 ${index % 2 === 0 ? 'justify-end' : 'justify-start'}`}>
                  {item.tags.map(tech => <Badge key={tech} variant="secondary">{tech}</Badge>)}
                </div>
              )}
            </div>
          </div>
...
const Grid2ColLayout = ({ items }: { items: PortfolioItem[] }) => (
  <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
    {items.map((item) => (
      <a key={item.id} href={item.link_url || "#"} rel="noopener noreferrer" target="_blank" className="group rounded-lg bg-blueprint-bg p-5 transition-all duration-200 hover:border-primary/80 hover:shadow-lg hover:shadow-primary/10">
        <div className="flex items-center justify-between">
          <h3 className="font-mono text-xl font-bold text-foreground transition-colors group-hover:text-primary">{item.title}</h3>
    ...
    {items.map((item) => {
      const content = (
        <div className="flex items-center justify-between gap-4">
          ...
          {item.date_to && <p className="flex-shrink-0 font-mono text-sm text-muted-foreground">{item.date_to}</p>}
        </div>
      );

      if (item.link_url) {
        return (
          <a key={item.id} href={item.link_url} target="_blank" rel="noopener noreferrer" className="block rounded-lg bg-card p-4 transition-colors hover:bg-secondary">
            {content}
          </a>
        );
      }
      ...
      default:
        ...
        return (
          <div className="text-center text-muted-foreground py-8">
            <p>Unsupported layout style "{layout_style}" for content type "{type}".</p>
          </div>
        );
    }
  };
