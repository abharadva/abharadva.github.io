...
const LinkForm = ({ link, onSave, onCancel }: { link: Partial<NavLink> | null, onSave: (data: Partial<NavLink>) => void, onCancel: () => void }) => {
  const [formData, setFormData] = useState({ label: '', href: '', display_order: 0 });
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <div><Label htmlFor="label">Label</Label><Input id="label" value={formData.label} onChange={(e) => setFormData(f => ({...f, label: e.target.value}))} required /></div>
      <div><Label htmlFor="href">Path (e.g., /about)</Label><Input id="href" value={formData.href} onChange={(e) => setFormData(f => ({...f, href: e.target.value}))} required /></div>
      <div><Label htmlFor="display_order">Display Order</Label><Input id="display_order" type="number" value={formData.display_order} onChange={(e) => setFormData(f => ({...f, display_order: parseInt(e.target.value, 10) || 0}))} /></div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage the main navigation links for your site.</p>
        </div>
        <Button onClick={() => { setEditingLink(null); setIsDialogOpen(true); }}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>{editingLink ? 'Edit' : 'Add'} Navigation Link</DialogTitle></DialogHeader>
          <LinkForm link={editingLink} onSave={handleSave} onCancel={() => setIsDialogOpen(false)} />
        </DialogContent>
      </Dialog>
    </div>
  );
