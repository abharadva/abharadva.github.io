...
import { CalendarDays, BookOpen, Flame, Trophy, TrendingUp, Clock } from "lucide-react";
        ...
        <div className="grid grid-flow-col grid-rows-7 gap-1.5 overflow-x-auto pb-2">
            ...
            return (
                ...
                <TooltipTrigger asChild>
                    <div className={cn("aspect-square rounded-[2px] w-3 h-3 sm:w-4 sm:h-4 transition-all hover:ring-2 hover:ring-ring hover:z-10", getColorClass(minutes))} />
                </TooltipTrigger>
                <TooltipContent className="text-xs">
                    <p className="font-bold">
                    {minutes > 0 ? `${minutes} mins` : "No study"}
                    </p>
                    <p className="text-muted-foreground">{format(day, "MMM do, yyyy")}</p>
                </TooltipContent>
                </Tooltip>
            );
            })}
...
export default function LearningDashboard({ sessions, topics }: { sessions: LearningSession[]; topics: LearningTopic[]; }) {
  ...
  const topicsMap = topics.reduce((acc, t) => ({...acc, [t.id]: t.title}), {} as Record<string, string>);
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto pb-10">
      <div className="flex items-end justify-between">
        <div>
            ...
            <p className="text-sm text-muted-foreground">Track your consistency and knowledge growth.</p>
        </div>
      </div>
      ...
      <Card className="overflow-hidden border-primary/10 shadow-sm">
        <CardHeader className="pb-4 border-b bg-muted/10">
            ...
            <CardDescription>Your daily learning volume over the last year.</CardDescription>
        </CardHeader>
        ...
        <div className="xl:col-span-2 flex flex-col justify-between gap-6">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 h-full">
                <Card className="bg-gradient-to-br from-primary/5 to-background border-primary/20 shadow-sm flex flex-col justify-between">
                    ...
                    <CardContent>
                        <div className="text-4xl font-bold tracking-tighter text-foreground">
                            {stats.totalHours}<span className="text-lg font-normal text-muted-foreground ml-1">hrs</span>
                        </div>
                        <p className="text-xs text-muted-foreground mt-1">Total tracked time</p>
                    </CardContent>
                </Card>
                ...
                <Card className="flex flex-col justify-between">
                    ...
                    <CardContent>
                        <div className="text-4xl font-bold tracking-tighter">
                            {sessions.filter(s => new Date(s.start_time) > subDays(new Date(), 7)).length}
                        </div>
                ...
                <Card className="flex flex-col justify-between">
                    ...
                    <CardContent>
                        <div className="text-4xl font-bold tracking-tighter">{topics.length}</div>
                        <p className="text-xs text-muted-foreground font-medium">Active learning paths</p>
                    </CardContent>
                </Card>
            </div>
            ...
            <Card className="flex-1 bg-secondary/5 border-dashed flex items-center justify-center min-h-[150px]">
                <div className="text-center text-muted-foreground">
                    ...
                    <p className="text-sm">Select a topic from the sidebar to view details.</p>
                </div>
            </Card>
        </div>
                ...
                <ScrollArea className="h-[350px]">
                    <div className="p-5 space-y-6">
                        {recentSessions.map((session, i) => (
                            <div key={session.id} className="relative pl-4 border-l border-border pb-1 last:pb-0 group">
                                <span className={cn(
                                    "absolute -left-[5px] top-1.5 h-2.5 w-2.5 rounded-full border-2 border-background transition-colors", 
                                    i === 0 ? "bg-primary" : "bg-muted-foreground group-hover:bg-primary/70"
                                )} />
