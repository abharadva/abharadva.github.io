...
export default function LearningDashboard({
  ...
  const { heatmapData, gridDays } = useMemo(() => {
    ...
    const data = sessions.reduce((acc: any, s) => {
    ...
    <div className="space-y-10 max-w-[1400px] mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-gradient-to-br from-primary/5 to-transparent border-primary/20">
          ...
          <CardContent>
            <div className="text-3xl font-black">{stats.totalHours}<span className="text-lg font-medium text-muted-foreground ml-1">hrs</span></div>
          </CardContent>
        </Card>
      ...
      <div className="space-y-4">
        ...
        <Card className="overflow-hidden border shadow-sm">
          <CardContent className="p-6 overflow-x-auto">
            <div className="min-w-[800px]"><Heatmap data={heatmapData} days={gridDays} /></div>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-lg font-bold flex items-center gap-2 text-foreground">
             <Star className="size-5 text-yellow-500 fill-yellow-500"/> Module Progress
          </h3>
        </div>
        ...
        {subjects.length === 0 ? (
          <div className="p-12 border-2 border-dashed rounded-2xl bg-muted/5 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-muted-foreground text-sm mt-1">Create a Subject to begin tracking your roadmap.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {subjects.map(subject => {
              const subTopics = topics.filter(t => t.subject_id === subject.id);
              const completed = subTopics.filter(t => t.status === "Mastered").length;
              const total = subTopics.length;
              const progress = total > 0 ? (completed / total) * 100 : 0;
              
              return (
                <Card key={subject.id} className="group hover:border-primary/50 transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
                  <CardHeader className="pb-3 border-b border-border/40 bg-muted/10">
                    <CardTitle className="truncate text-base" title={subject.name}>{subject.name}</CardTitle>
                  </CardHeader>
                  <CardContent className="pt-4">
                    <div className="flex justify-between items-end mb-2">
                       <div className="flex flex-col">
                         <span className="text-[10px] uppercase font-bold text-muted-foreground tracking-wider">Completion</span>
                         <span className="text-sm font-mono text-foreground">{completed} / {total}</span>
                       </div>
