        ...
        {subjects.map((subject) => (
          <AccordionItem value={subject.id} key={subject.id}>
            <div className="group flex items-center justify-between pr-1">
              <AccordionTrigger className="flex-1 py-2 text-sm font-semibold hover:no-underline">
                {subject.name}
              </AccordionTrigger>
              <DropdownMenu>
                    ...
                    return (
                      <div key={topic.id} className="group flex items-center">
                        <Button
                          ...
                          className="h-8 flex-1 justify-start px-2 text-left"
                          onClick={() => onSelectTopic(topic)}
                        >
                          <div className="relative mr-2">
                            <BrainCircuit className="size-3.5 shrink-0" />
                            {isCurrentlyLearning && (
                              <span className="absolute -right-1 -top-1 flex h-2 w-2 rounded-full bg-primary animate-ping" />
                            )}
                          </div>
