...
const getStatusIcon = (status: string) => {
  switch (status) {
    case "Mastered": return <Trophy className="size-3.5 text-yellow-500" />;
    case "Practicing": return <CheckCircle2 className="size-3.5 text-green-500" />;
    case "Learning": return <Timer className="size-3.5 text-blue-500" />;
    default: return <Circle className="size-3.5 text-muted-foreground/50" />;
  }
};
...
export default function SubjectTopicTree({
  ...
  const filteredSubjects = useMemo(() => {
    ...
    return subjects.filter(subject => {
      ...
      const hasMatchingTopics = topics.some(t => 
        t.subject_id === subject.id && t.title.toLowerCase().includes(searchQuery.toLowerCase())
      );
      ...
      <ScrollArea className="flex-1 px-2 py-2">
        ...
        >
          {filteredSubjects.map((subject) => {
            ...
            const progress = subjectTopics.length > 0 ? Math.round((masteredCount / subjectTopics.length) * 100) : 0;
                ...
                <div className="group flex items-center justify-between py-1 pr-1 mb-1">
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    ...
                    {!searchQuery && subjectTopics.length > 0 && (
                        <span className="text-[10px] text-muted-foreground font-mono mr-1">{progress}%</span>
                    )}
                    <DropdownMenu>
                    ...
                    {visibleTopics.map((topic) => {
                      ...
                      return (
                        <div key={topic.id} className="group relative flex items-center">
                          ...
                          {/* Quick Edit Action */}
                          <div className={cn("absolute right-1 flex items-center", isActive ? "opacity-100" : "opacity-0 group-hover:opacity-100 transition-opacity")}>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <Button variant="ghost" size="icon" className="h-6 w-6">
                                ...
                                <DropdownMenuContent align="start">
                                    <DropdownMenuItem onClick={() => onEditTopic(topic)}>Edit</DropdownMenuItem>
                                    <DropdownMenuItem className="text-destructive" onClick={() => onDeleteTopic(topic.id)}>Delete</DropdownMenuItem>
                                </DropdownMenuContent>
                            </DropdownMenu>
                          </div>
                        </div>
