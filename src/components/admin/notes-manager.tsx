...
import { useGetNotesQuery, useAddNoteMutation, useUpdateNoteMutation, useDeleteNoteMutation } from "@/store/api/adminApi";
...
export default function NotesManager() {
  ...
  const handleCancel = () => { setIsCreating(false); setEditingNote(null); };
  ...
  return (
    <div className="space-y-6">
      ...
      {(isLoading && !notes.length) && <div className="flex justify-center py-10"><Loader2 className="h-8 w-8 animate-spin text-muted-foreground" /></div>}
      {!!error && <Alert variant="destructive"><AlertTitle>Error</AlertTitle><AlertDescription>{error && typeof error === 'object' && 'message' in error
        ? String((error as { message: unknown }).message)
        : 'Failed to load notes'}</AlertDescription></Alert>}
                  ...
                  <CardFooter className="flex-col items-start gap-3 border-t pt-4">
                    ...
                    <div className="flex w-full items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" className="size-8" disabled={isMutating} onClick={() => handleTogglePin(note)} title={note.is_pinned ? "Unpin" : "Pin"}>{note.is_pinned ? <PinOff className="size-4 text-primary" /> : <Pin className="size-4" />}</Button>
                        <Button variant="ghost" size="icon" className="size-8" disabled={isMutating} onClick={() => handleEditNote(note)} title="Edit"><Edit className="size-4" /></Button>
                        <Button variant="ghost" size="icon" className="size-8 hover:bg-destructive/10 hover:text-destructive" disabled={isMutating} onClick={() => handleDeleteNote(note.id)} title="Delete"><Trash2 className="size-4" /></Button>
                      </div>
                    </div>
                  </CardFooter>
                </Card>
