...
import { CheckCircle, AlertCircle, Loader2, ShieldCheck, ShieldAlert, KeyRound, Smartphone } from "lucide-react";
...
export default function SecuritySettings() {
  ...
  const handleUnenroll = async (factorId: string) => {
    if (!confirm("Are you sure? Removing your only 2FA method may lock you out until you set it up again upon next login.")) return;
    try {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-4xl space-y-8">
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-4">
            {mfaEnabled ? <ShieldCheck className="h-10 w-10 text-green-500" /> : <ShieldAlert className="h-10 w-10 text-yellow-500" />}
            <div>
            ...
            <Badge variant={mfaEnabled ? "default" : "secondary"} className={mfaEnabled ? "bg-green-500/15 text-green-600" : ""}>
        ...
        <CardContent>
          {!mfaEnabled && !isLoading && (
            <div className="text-center p-6 border bg-secondary/30 rounded-lg">
              ...
              <p className="text-sm text-muted-foreground mt-1 mb-4">Protect your account from unauthorized access.</p>
              <Button onClick={() => router.push("/admin/setup-mfa")}><Smartphone className="mr-2 size-4"/> Enable 2FA Now</Button>
            </div>
          )}
          {mfaEnabled && factors.length > 0 && (
            <>
              <h4 className="text-sm font-semibold mb-2">Registered Authenticators</h4>
              <Table>
                <TableHeader><TableRow><TableHead>Name</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Action</TableHead></TableRow></TableHeader>
                <TableBody>
                  {factors.map((factor) => (
                    <TableRow key={factor.id}>
                      <TableCell className="font-medium">{factor.friendly_name || `Authenticator`}</TableCell>
                      <TableCell><Badge variant={factor.status === 'verified' ? 'default' : 'secondary'}>{factor.status}</Badge></TableCell>
                      <TableCell className="text-right">
                        <Button onClick={() => handleUnenroll(factor.id)} variant="destructive" size="sm" disabled={isLoading}>Remove</Button>
                      </TableCell>
                    </TableRow>
                  ))}
                </TableBody>
        ...
        {mfaEnabled && (
          <CardFooter className="border-t pt-4">
            <Button variant="outline" size="sm" onClick={() => router.push("/admin/setup-mfa")} disabled={isLoading}>Add Another Method</Button>
          </CardFooter>
        )}
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><KeyRound className="size-5 text-primary" /> Change Password</CardTitle>
