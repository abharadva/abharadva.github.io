...
import { CheckCircle, XCircle, Loader2, FlaskConical, AlertTriangle, Timer, FileClock } from "lucide-react";
...
export default function AdminTestPage() {
  ...
  const { data: siteSettingsData, isLoading: isLoadingSettings } = useGetSiteSettingsQuery();
  const [updateSiteSettings] = useUpdateSiteSettingsMutation();

  const addLog = (log: TestLog) => setLogs(prev => [...prev.filter(l => l.name !== log.name), log]);
  ...
  const runTest = async (name: string, testFn: () => Promise<any>) => {
    ...
    try {
      ...
      addLog({ name, status: 'fail', message, duration: Date.now() - startTime });
  ...
  const handleRunAllTests = async () => {
    ...
    try {
      ...
      await runTest("Goals: Update", () => saveGoal({ id: goalRes.id, current_amount: 50 }).unwrap());
      ...
      await runTest("Topics: Update", () => saveTopic({ id: topicRes.id, status: "Learning" }).unwrap());
      ...
      const itemRes = await runTest("Items: Create", () => savePortfolioItem({ section_id: sectionRes.id, title: `[TEST] ${testId}` }).unwrap());
      await runTest("Items: Update", () => savePortfolioItem({ id: itemRes.id, subtitle: "[U]" }).unwrap());
      await runTest("Items: Delete", () => deletePortfolioItem(itemRes.id).unwrap());
      ...
      await runTest("NavLinks: Update", () => saveNavLink({ id: navRes.id, is_visible: false }).unwrap());
      ...
      if (siteSettingsData) {
        ...
        await runTest("SiteSettings: Update", () => updateSiteSettings({ portfolio_mode: newMode }).unwrap());
        await runTest("SiteSettings: Revert", () => updateSiteSettings({ portfolio_mode: originalMode }).unwrap());
      } else {
        addLog({ name: 'SiteSettings: Skipped', status: 'fail', message: 'Initial settings not loaded.' });
      }
      ...
      toast.error("A test failed. Stopping execution.", { description: error.message });
    } finally {
      setIsTesting(false);
      stopTestingRef.current = false;
      store.dispatch(adminApi.util.invalidateTags(["AdminPosts", "Notes", "Tasks", "Transactions", "Recurring", "Goals", "Learning", "PortfolioContent", "Assets", "Navigation", "SiteContent", "Calendar", "Dashboard"]));
    }
  };
  ...
  if (isAuthLoading) return <AdminLayout>
    <LoadingSpinner />
  </AdminLayout>;
      ...
      <div className="space-y-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Run a full Create, Read, Update, Delete cycle on all database tables.</p>
          </div>
          <Button onClick={isTesting ? () => (stopTestingRef.current = true) : handleRunAllTests} disabled={isLoadingSettings} size="lg" variant={isTesting ? "destructive" : "default"}>
            {isTesting ? <Loader2 className="mr-2 size-4 animate-spin" /> : <FlaskConical className="mr-2 size-4" />}
            {isTesting ? "Stop Tests" : (isLoadingSettings ? "Loading..." : "Run All Tests")}
          </Button>
        </div>
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertTitle>Warning</AlertTitle>
          <AlertDescription>This performs live database operations. While it cleans up after itself, running this on a production database is not recommended.</AlertDescription>
        </Alert>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-500">{summary.success} / {summary.total}</div>
            </CardContent>
          </Card>
          <Card>
            <CardHeader className="flex-row items-center justify-between pb-2">
          ...
          <CardContent>
            <ScrollArea className="h-96 w-full rounded-md border">
              <Table>
                ...
                <TableBody>
                  {logs.map((log, index) => (
                    <TableRow key={index} className={log.status === 'fail' ? 'bg-destructive/10' : ''}>
                      <TableCell>
                      ...
                      <TableCell className="font-mono text-xs">{log.duration !== undefined ? `${log.duration}ms` : '...'}</TableCell>
                  ...
                  {logs.length === 0 && <TableRow>
                    <TableCell colSpan={4} className="h-24 text-center">Click "Run All Tests" to begin.</TableCell>
                  </TableRow>}
                </TableBody>
              </Table>
            </ScrollArea>
