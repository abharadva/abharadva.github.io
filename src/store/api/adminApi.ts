...
import type { Event, BlogPost, Note, Task, SubTask, Transaction, RecurringTransaction, FinancialGoal, LearningSubject, LearningTopic, LearningSession, PortfolioSection, PortfolioItem, SiteContent } from '@/types'

type NavLink = { id: string; label: string; href: string; display_order: number; is_visible: boolean };
type StorageAsset = { id: string; file_name: string; file_path: string; mime_type: string | null; size_kb: number | null; alt_text: string | null; used_in: { type: string; id: string }[] | null; created_at: string; };
...
export const adminApi = createApi({
    ...
    tagTypes: ['Notes', 'Tasks', 'Transactions', 'Recurring', 'Goals', 'Learning', 'PortfolioContent', 'Assets', 'Navigation', 'SiteSettings', 'AdminPosts', 'Calendar', 'Dashboard', 'MFA'],
    endpoints: (builder) => ({
        // Auth & Security
        ...
        getDashboardData: builder.query<any, void>({
            queryFn: async () => {
                ...
                const [
                    ...
                    { data: pinnedNotesData }, { data: monthlyTransactionsData }, { data: totalViewsRes },
                    { count: tasksCompletedCount }, { data: learningSessionsData }, { count: topicsInProgressCount },
                ] = results as any[];
                ...
                const totalMinutes = learningSessionsData?.reduce((sum: number, s: any) => sum + (s.duration_minutes || 0), 0) || 0;
                ...
                const data = {
                    stats: {
                        ...
                        monthlyNet: monthlyEarnings - monthlyExpenses, totalBlogViews: totalViewsRes || 0,
            ...
            providesTags: ['Dashboard', 'AdminPosts', 'Notes', 'Tasks', 'Transactions', 'Learning', 'PortfolioContent'],
        }),

        // Calendar Data
        getCalendarData: builder.query<{ baseEvents: CalendarItem[], recurring: RecurringTransaction[] }, { start: string, end: string }>({
            queryFn: async ({ start, end }) => {
                const [calendarDataRes, recurringRes] = await Promise.all([
                    supabase.rpc('get_calendar_data', { start_date_param: start, end_date_param: end }),
                ...
                return { data: { baseEvents: calendarDataRes.data, recurring: recurringRes.data } };
            },
        ...
        getAdminBlogPosts: builder.query<BlogPost[], void>({
            ...
            providesTags: (result) => result ? [...result.map(({ id }) => ({ type: 'AdminPosts' as const, id })), { type: 'AdminPosts', id: 'LIST' }] : [{ type: 'AdminPosts', id: 'LIST' }],
        }),
        ...
        updateBlogPost: builder.mutation<BlogPost, Partial<BlogPost>>({
            ...
            invalidatesTags: (result, error, arg) => [{ type: 'AdminPosts', id: arg.id }],
        }),
        deleteBlogPost: builder.mutation<{ post: BlogPost }, BlogPost>({
            queryFn: async (post) => {
                if (post.cover_image_url && post.cover_image_url.includes(process.env.NEXT_PUBLIC_SUPABASE_URL!)) {
                    const pathSegments = post.cover_image_url.split('/');
                    const imagePath = pathSegments.slice(pathSegments.indexOf(BUCKET_NAME) + 1).join('/');
            ...
            invalidatesTags: (result, error, arg) => [{ type: 'AdminPosts', id: 'LIST' }, { type: 'AdminPosts', id: arg.id }],
        }),
        ...
        getTasks: builder.query<Task[], void>({
            queryFn: async () => {
                const { data, error } = await supabase.from('tasks').select('*, sub_tasks(*)').order('created_at', { ascending: false });
                if (error) return { error };
                return { data };
            },
            providesTags: ['Tasks'],
        ...
        updateSubTask: builder.mutation<SubTask, Partial<SubTask>>({
            queryFn: async (subTask) => {
                ...
                const { data, error } = await supabase.from('sub_tasks').update(updateData).eq('id', id!).select().single();
                if (error) return { error };
                return { data };
            },
            invalidatesTags: ['Tasks'],
        ...
        // Finance
        getFinancialData: builder.query<{ transactions: Transaction[], goals: FinancialGoal[], recurring: RecurringTransaction[] }, void>({
            queryFn: async () => {
                ...
                return { data: { transactions: tranRes.data, goals: goalRes.data, recurring: recurRes.data } };
            },
        ...
        saveTransaction: builder.mutation<Transaction, Partial<Transaction>>({
            queryFn: async (transaction) => {
                ...
                const promise = id ? supabase.from('transactions').update(updateData).eq('id', id) : supabase.from('transactions').insert(updateData);
        ...
        saveRecurring: builder.mutation<RecurringTransaction, Partial<RecurringTransaction>>({
            queryFn: async (rec) => {
                const { id, ...updateData } = rec;
                const promise = id ? supabase.from('recurring_transactions').update(updateData).eq('id', id) : supabase.from('recurring_transactions').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        saveGoal: builder.mutation<FinancialGoal, Partial<FinancialGoal>>({
            queryFn: async (goal) => {
                ...
                const promise = id ? supabase.from('financial_goals').update(updateData).eq('id', id) : supabase.from('financial_goals').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        addFundsToGoal: builder.mutation<FinancialGoal, { goal: FinancialGoal, amount: number }>({
            queryFn: async ({ goal, amount }) => {
                const newCurrentAmount = goal.current_amount + amount;
                const { data, error } = await supabase.from('financial_goals').update({ current_amount: newCurrentAmount }).eq('id', goal.id).select().single();
                if (error) return { error };
                ...
                if (transError) console.warn("Goal updated, but failed to create a matching transaction.", transError);
        ...
        manageCategory: builder.mutation<any, { type: 'edit' | 'merge' | 'delete', oldName: string, newName?: string }>({
            queryFn: async ({ type, oldName, newName }) => {
                ...
                if (type === 'edit' && newName) { rpcName = 'rename_transaction_category'; params = { old_name: oldName, new_name: newName }; }
                else if (type === 'merge' && newName) { rpcName = 'merge_transaction_categories'; params = { source_name: oldName, target_name: newName }; }
                else if (type === 'delete') { rpcName = 'delete_transaction_category'; params = { category_name: oldName }; }

                if (!rpcName) return { error: { message: 'Invalid action', details: '', hint: '', code: '400' } };
        ...
        // Learning
        getLearningData: builder.query<{ subjects: LearningSubject[], topics: LearningTopic[], sessions: LearningSession[] }, void>({
            queryFn: async () => {
                const [subjectsRes, topicsRes, sessionsRes] = await Promise.all([
                    ...
                    supabase.from("learning_sessions").select("*").order("start_time", { ascending: false }).limit(100),
                ]);
                if (subjectsRes.error || topicsRes.error || sessionsRes.error) {
                    return { error: subjectsRes.error || topicsRes.error || sessionsRes.error };
                }
                return { data: { subjects: subjectsRes.data, topics: topicsRes.data, sessions: sessionsRes.data } };
            },
            providesTags: ['Learning'],
        }),
        addLearningSession: builder.mutation<LearningSession, Partial<LearningSession>>({
        ...
        updateLearningSession: builder.mutation<LearningSession, Partial<LearningSession>>({
            queryFn: async (session) => {
                const { id, ...updateData } = session;
                const { data, error } = await supabase.from('learning_sessions').update(updateData).eq('id', id!).select().single();
                if (error) return { error };
                return { data };
            },
            invalidatesTags: ['Learning'],
        ...
        saveSubject: builder.mutation<LearningSubject, Partial<LearningSubject>>({
            queryFn: async (subject) => {
                ...
                const promise = id ? supabase.from('learning_subjects').update(updateData).eq('id', id) : supabase.from('learning_subjects').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        saveTopic: builder.mutation<LearningTopic, Partial<LearningTopic>>({
            queryFn: async (topic) => {
                ...
                const promise = id ? supabase.from('learning_topics').update(updateData).eq('id', id) : supabase.from('learning_topics').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        getSiteSettings: builder.query<{ identity: SiteContent, settings: { portfolio_mode: string } }, void>({
            queryFn: async () => {
                ...
                if (identityRes.error || settingsRes.error) return { error: identityRes.error || settingsRes.error };
                return { data: { identity: identityRes.data as SiteContent, settings: settingsRes.data } };
            },
            providesTags: ['SiteSettings'],
        }),
        updateSiteSettings: builder.mutation<void, { identity: Partial<SiteContent>; settings: { portfolio_mode: string } }>({
        ...
        saveNavLink: builder.mutation<NavLink, Partial<NavLink>>({
            queryFn: async (link) => {
                ...
                const promise = id ? supabase.from('navigation_links').update(updateData).eq('id', id) : supabase.from('navigation_links').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        getPortfolioContent: builder.query<PortfolioSection[], void>({
            queryFn: async () => {
                const { data, error } = await supabase.from("portfolio_sections").select(`*, portfolio_items (*)`).order("page_path").order("display_order", { ascending: true }).order("display_order", { foreignTable: "portfolio_items", ascending: true });
                if (error) return { error };
                return { data };
            },
        ...
        saveSection: builder.mutation<PortfolioSection, Partial<PortfolioSection>>({
            queryFn: async (section) => {
                ...
                const promise = id ? supabase.from('portfolio_sections').update(updateData).eq('id', id) : supabase.from('portfolio_sections').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        savePortfolioItem: builder.mutation<PortfolioItem, Partial<PortfolioItem>>({
            queryFn: async (item) => {
                ...
                const promise = id ? supabase.from('portfolio_items').update(updateData).eq('id', id) : supabase.from('portfolio_items').insert(updateData);
                const { data, error } = await promise.select().single();
                if (error) return { error };
                return { data };
            },
        ...
        updateSectionOrder: builder.mutation<void, string[]>({
            queryFn: async (sectionIds) => {
                const { error } = await supabase.rpc('update_section_order', { section_ids: sectionIds });
                if (error) return { error };
                return { data: undefined };
            },
            invalidatesTags: ['PortfolioContent'],
        ...
        getAssets: builder.query<StorageAsset[], void>({
            queryFn: async () => {
                const { data, error } = await supabase.from('storage_assets').select('*').order('created_at', { ascending: false });
                if (error) return { error };
                return { data };
            },
            providesTags: ['Assets'],
        ...
        addAsset: builder.mutation<StorageAsset, Partial<StorageAsset>>({
            queryFn: async (asset) => {
                const { data, error } = await supabase.from('storage_assets').insert(asset).select().single();
                if (error) return { error };
                return { data };
            },
        ...
        updateAsset: builder.mutation<StorageAsset, Partial<StorageAsset>>({
            queryFn: async (asset) => {
                ...
                const { data, error } = await supabase.from('storage_assets').update(updateData).eq('id', id!).select().single();
                if (error) return { error };
                return { data };
            },
            invalidatesTags: ['Assets'],
        ...
        deleteAsset: builder.mutation<{ asset: StorageAsset }, StorageAsset>({
            queryFn: async (asset) => {
                const { error: storageError } = await supabase.storage.from(process.env.NEXT_PUBLIC_BUCKET_NAME || 'blog-assets').remove([asset.file_path]);
                if (storageError) return { error: storageError };
                const { error: dbError } = await supabase.from('storage_assets').delete().eq('id', asset.id);
