...
import type { BlogPost, GitHubRepo, PortfolioSection, SiteContent } from '@/types'
...
export const publicApi = createApi({
  ...
  tagTypes: ['SiteContent', 'Posts', 'Post', 'Portfolio', 'Navigation', 'SiteSettings'],
  endpoints: (builder) => ({
    ...
    getPublishedBlogPosts: builder.query<BlogPost[], void>({
      ...
      providesTags: (result) => result ? [...result.map(({ id }) => ({ type: 'Posts' as const, id })), { type: 'Posts', id: 'LIST' }] : [{ type: 'Posts', id: 'LIST' }],
    }),
    getBlogPostBySlug: builder.query<BlogPost, string>({
      queryFn: async (slug) => {
        ...
        if (!data) return { error: { message: 'Not Found', details: '', hint: '', code: '404' } };
        return { data };
      },
    ...
    incrementPostView: builder.mutation<void, string>({
      queryFn: async (postId) => {
        const { error } = await supabase.rpc('increment_blog_post_view', { post_id_to_increment: postId });
        if (error) return { error };
        return { data: undefined };
      },
      invalidatesTags: (result, error, postId) => [{ type: 'Post', id: postId }, { type: 'Posts', id: 'LIST' }],
    }),
    getSectionsByPath: builder.query<PortfolioSection[], string>({
      queryFn: async (pagePath) => {
        const { data, error } = await supabase.from('portfolio_sections').select('*, portfolio_items(*)').eq('page_path', pagePath).eq('is_visible', true).order('display_order').order('display_order', { foreignTable: 'portfolio_items' });
        if (error) return { error };
        return { data };
      },
      providesTags: (result, error, path) => [{ type: 'Portfolio', id: path }],
    }),
    getGitHubRepos: builder.query<GitHubRepo[], { username: string; sort_by: string; projects_per_page: number; page: number; exclude_forks: boolean; exclude_archived: boolean; exclude_profile_repo: boolean; min_stars: number }>({
      queryFn: async (args) => {
        const { username, sort_by, projects_per_page, page, exclude_forks, exclude_archived, exclude_profile_repo, min_stars } = args;
        ...
        try {
          ...
          return { error: { message: error.message, details: '', hint: '', code: 'FETCH_ERROR' } };
        }
      },
    }),
  }),
